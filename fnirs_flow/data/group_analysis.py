"""Group design, contrast, GLM, correction, and permutation statistics."""
            ...
            for levels in product(*contrast_levels):
                ...
                for index, row in enumerate(rows):
                    matrix[index].append(
                        1.0
                        if all(
                            str(row.get(factor, "")) == level
                            for factor, level in zip(combo, levels, strict=True)
                        )
                        else 0.0
                    )
    return column_names, matrix
...
def fit_group_glm(
    ...
    *,
    ...
    random_seed: int = 0,
    permutation_chunk_size: int = 256,
    ...
    sorted_features = sorted(rows_by_feature.items())
    stream_count = max(1, len(sorted_features) * max(1, len(contrast_specs)))
    permutation_streams = iter(np.random.SeedSequence(random_seed).spawn(stream_count))
    for feature_key, indexed_rows in sorted_features:
        ...
        for spec in contrast_specs:
            permutation_rng = np.random.default_rng(next(permutation_streams))
            if spec.contrast_type == "F":
                ...
                permutation_p = _permutation_f_p_value(
                    ...
                    rng=permutation_rng,
                    x_pinv=np.linalg.pinv(x_feature),
                    xtx_inv=xtx_inv,
                ...
                contrast_rows.append(
                    {
                        **feature,
                        ...
                        "permutation_stream": "seed_sequence_child",
                        "permutation_chunk_size": permutation_chunk_size,
                        "permutation_executed": permutation_count,
                    }
                )
                effect_rows.append(
                    {
...
def _permutation_t_p_value(
    ...
    *,
    ...
    for index in range(n_permutations):
        if index % max(1, chunk_size) == 0 and cancel_check is not None and cancel_check():
            raise RuntimeError("Permutation computation cancelled")
        shuffled = rng.permutation(y)
        beta = x_pinv @ shuffled
