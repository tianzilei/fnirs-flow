...
def parse_vendor_processed_hb(
    ...
    for i, name in enumerate(header_names):
        match = _CHANNEL_RE.match(name.strip())
        if not match:
            continue
        chrom = {"oxyhb": "hbo", "deoxyhb": "hbr", "totalhb": "hbt"}[match.group(2).casefold()]
        key = (int(match.group(1)), chrom)
        if key in fields:
    ...
    finite_channels = np.isfinite(values[:, [fields[(n, c)] for n in channels_numbers for c in ("hbo", "hbr")]])
    ...
    hbo = np.vstack([values[:, fields[(n, "hbo")]] for n in channels_numbers])
    hbr = np.vstack([values[:, fields[(n, "hbr")]] for n in channels_numbers])
    hbt = (
        np.vstack([values[:, fields[(n, "hbt")]] for n in channels_numbers])
        if all((n, "hbt") in fields for n in channels_numbers)
        else None
    )
    ...
    hbt_metrics: dict[str, object] = {
        "hbt_mae": None,
        "hbt_rmse": None,
        "hbt_max_abs_error": None,
        "hbt_error_quantiles": (),
        "hbt_tolerance_exceedance_fraction": None,
    }
    if hbt is not None:
        err = hbt - (hbo + hbr)
        hbt_metrics = {
    ...
    provenance = InputProvenance(
        ...
        hbt_status,
        hbt_metrics["hbt_mae"],
        hbt_metrics["hbt_rmse"],
        hbt_metrics["hbt_max_abs_error"],
        hbt_metrics["hbt_error_quantiles"],
        hbt_metrics["hbt_tolerance_exceedance_fraction"],
        tuple(warnings),
        "pass",
    )
    qc = ParserQC(
