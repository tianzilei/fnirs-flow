"""Managed adapters derived from the owner's private fNIRS utility scripts.

The original scripts live in ``tianzilei/MainCodeRepo`` (Apache-2.0).  These
adapters keep the useful parsing logic while removing GUI automation,
hard-coded paths, destructive moves, and hard-coded participant attributes.
"""
    ...
    *,
    ...
    paths = sorted(
        (
            ...
            if path.is_file() and (not normalized_extensions or path.suffix.lower() in normalized_extensions)
        ),
...
def inspect_nirs_spm_headers(
    ...
    *,
    ...
    if source.is_file():
        ...
        paths = sorted((path for path in candidates if path.is_file()), key=lambda path: path.as_posix().lower())
        source_root = source
    else:
