...
The validation flow is export -> verify -> import -> relink -> rerun. The rerun
uses a project-local isolated `.venv-mne` environment and an independent output
directory. The exported package uses `external-data://ds007738/` and does not
include `/Volumes/` or `/Users/` machine-absolute paths. This validation does
not claim cross-operating-system reproducibility.
"""
