...
`qc_failed` is recorded as a quality exclusion, not as a software execution
failure. Resting-state runs without task events are recorded as not applicable
for this analysis. Missing events files and corrupt SNIRF metadata retain
separate reason codes.

Machine-readable manifests are in `outputs/ds007738_full_analysis/logs/`.
"""
